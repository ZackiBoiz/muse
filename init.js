...
export async function init(args, state) {
  ...
  if (file) {
    ...
    // if no parameter and empty load cached program else load default program
...
async function sendData(data) {
  ...
  const upload = await fetch(
    "https://sound-sampler.maxwofford.repl.co/upload-sound",
    {
      method: "POST",
      body,
    }
  ...
  const url =
    "https://sound-sampler.maxwofford.repl.co/" + upload.data.filename;
  const sampleObj = {};
  const name = makeId(4);
